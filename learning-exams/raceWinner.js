function main (){
    ...
    console.log("the winner is "+ getWinner(driversData,idealLap,pitStopTime))
}

function getWinner(driversData, idealLap, pitStopTime) {
    let winner = " ";
    let min = Infinity;
    debugger ;for (let i = 0; i < driversData.length; i++) {
        const teamScore = (driversData[i].score * 0.6 + driversData[i].team.score * 0.4);
        let raceTime = 0;

        for (let j = 0; j < driversData[i].strategy.length; j++) {

            for (let lapNumber = 1; lapNumber <= driversData[i].strategy[j].laps; lapNumber++) {
                const tiresBase = driversData[i].strategy[j].tire.base;
                const tiresWear = driversData[i].strategy[j].tire.wearRate;
